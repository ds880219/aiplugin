...
ASErr AnnotatorPlugin::GoMenuItem(AIMenuMessage* message)
{
	...
	try 
	{
		if (message->menuItem == fAboutPluginMenu) {
			SDKAboutPluginsHelper aboutPluginsHelper;
			aboutPluginsHelper.PopAboutBox(message, "About", "This is IllustratorCLI");
		}
	}
	catch (ai::Error& ex) {
		result = ex;
...
ASErr AnnotatorPlugin::AddTool(SPInterfaceMessage *message)
{
	...
	try {
		ASErr			error = kNoErr;
		AIAddToolData	data;
        data.title = ai::UnicodeString::FromRoman("IllustratorCLI");
		data.tooltip = ai::UnicodeString::FromRoman("IllustratorCLI");
...
ASErr AnnotatorPlugin::AddAnnotator(SPInterfaceMessage *message)
{
	ASErr result = kNoErr;
	try {
		result = sAIAnnotator->AddAnnotator(message->d.self, "IllustratorCLI", &fAnnotatorHandle);
		aisdk::check_ai_error(result);
...
ASErr AnnotatorPlugin::AddNotifier(SPInterfaceMessage *message)
{
	ASErr result = kNoErr;
	try {
		result = sAINotifier->AddNotifier(fPluginRef, "IllustratorCLI", kAIArtSelectionChangedNotifier, &fNotifySelectionChanged);
		aisdk::check_ai_error(result);
		result = sAINotifier->AddNotifier(fPluginRef, "IllustratorCLI", kAIApplicationShutdownNotifier, &fShutdownApplicationNotifier);
		aisdk::check_ai_error(result);
